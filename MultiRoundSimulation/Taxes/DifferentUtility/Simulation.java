/**********
* MULTI-ROUND SIMULATION (TAXES, DIFFERENT UTILITY)
	...
	*/
	public static void haggling(Agent buyer, Agent seller){
		...
		while(!consensus && t <= maxTimeForHaggling){
			...
			if(t%2 == 1){
				double bPayoff = buyer.findUtility(buyer.getCash()-cash*Math.pow(d1, t-1), buyer.getWheat()+wheat*Math.pow(d1, t-1)*(1-tax));
				...
				while(bPayoff <= buyer.getUtility() && i<100){
					cash = Math.random() * surplusCash;
					wheat = Math.random() * surplusWheat;
					bPayoff = buyer.findUtility(buyer.getCash()-cash*Math.pow(d1, t-1), buyer.getWheat()+wheat*Math.pow(d1, t-1)*(1-tax));
					i++;
				}
				
				//see if the amount works for the seller
				double sPayoff = seller.findUtility(seller.getCash()+cash*Math.pow(d2, t-1)*(1-tax), seller.getWheat()-wheat*Math.pow(d2, t-1));
				...
				while(sPayoff <= seller.getUtility() && i<100){
					cash = Math.random() * surplusCash;
					wheat = Math.random() * surplusWheat;
					sPayoff = seller.findUtility(seller.getCash()+cash*Math.pow(d2, t-1)*(1-tax), seller.getWheat()-wheat*Math.pow(d2, t-1));
					i++;
				}
		...
		if(consensus == true){
			...
			buyer.setWheat(buyer.getWheat() + wheat*(1-tax));
			govtWheat += wheat*tax;

			seller.setCash(seller.getCash() + cash*(1-tax));
