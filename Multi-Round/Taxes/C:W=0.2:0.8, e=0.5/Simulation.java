...
public class Simulation{
	...
	double govtCash = 0;
	double govtWheat = 0;
	static PrintWriter govt;

	public Simulation(int numAgents, int rounds, int maxTime, double x, double taxIn, int integer){
		maxRounds = rounds;
		...
		try{
			...
			govt = new PrintWriter(new BufferedWriter(new FileWriter("DataFiles/Govt.txt", true)));
		} catch(IOException e){
		...
		govt.println(govtCash+"\t"+govtWheat);
	...
	public void haggling(Agent buyer, Agent seller){
		...
		while(!consensus && t <= maxTimeForHaggling){
			...
			if(t%2 == 1){
				...
				while(buyer.findChangeInUtility(cash, (1-tax)*wheat, true) <= buyer.getUtility() && i<100){
					cash = Math.random() * surplusCash;
					wheat = Math.random() * surplusWheat;
					i++;
				}

				if(seller.findChangeInUtility((1-tax)*cash, wheat, false)>seller.getUtility() && buyer.findChangeInUtility(cash, (1-tax)*wheat, true)>buyer.getUtility()){
					consensus = true;
				}
				...
				//keep on trying to find an amount that works for the buyer in terms of payoff
				while(seller.findChangeInUtility((1-tax)*cash, wheat, false) <= seller.getUtility() && i<100){
					cash = Math.random() * surplusCash;
					wheat = Math.random() * surplusWheat;
					i++;
				}
