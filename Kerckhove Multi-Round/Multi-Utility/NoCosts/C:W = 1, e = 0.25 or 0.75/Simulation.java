...
public class Simulation{
	...
	*/
	public void haggling(Agent buyer, Agent seller){
		...
		double totalCash = buyer.getCash() + seller.getCash(); //round(buyer.getCash() + seller.getCash(), 4);
		double totalWheat = buyer.getWheat() + seller.getWheat(); //round(buyer.getWheat() + seller.getWheat(), 4);
		...
		if(buyer.getUtility()+seller.getUtility()<buyer.findUtility(totalCash, totalWheat)){
			...
			double bc = round((1+bu-su)*totalCash/2, 4);
			double bw = round((1+bu-su)*totalWheat/2, 4);
			double sc = round((1+su-bu)*totalCash/2, 4);
			double sw = round((1+su-bu)*totalWheat/2, 4);

			double buA = buyer.findUtility(bc, bw);
			double suA = seller.findUtility(sc, sw);

			// double bu = buyer.findUtility(buyer.getCash()/totalCash, buyer.getWheat()/totalWheat);
			// double su = seller.findUtility(seller.getCash()/totalCash, seller.getWheat()/totalWheat);


			if(buA > buyer.getUtility() && suA > seller.getUtility()){
				double rate = (originalBC-bc)/(originalSW-sw);
				...
				if(!(Double.isNaN(rate) || rate==0)){
					...
					if(buyer.getExponent()==seller.getExponent()){
						if(buyer.getExponent() == 0.25){
							tracker1++;
							rates.println(tracker1+" "+rate + " " + 1);
						} else {
							tracker2++;
							rates.println(tracker2+" "+rate+ " " + 2);
						}
					} else {
						tracker3++;
						rates.println(tracker3+" "+rate+ " " + 3);
					}
				}
			}
