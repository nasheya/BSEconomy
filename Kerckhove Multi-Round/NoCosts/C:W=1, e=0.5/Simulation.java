...
public class Simulation{
	...
	*/
	public void haggling(Agent buyer, Agent seller){
		...
		double totalCash = buyer.getCash() + seller.getCash();
		double totalWheat = buyer.getWheat() + seller.getWheat();
		...
		if(buyer.getUtility()+seller.getUtility()<buyer.findUtility(totalCash, totalWheat)){
			...
			double buA = buyer.findUtility((1+bu-su)*totalCash/2, (1+bu-su)*totalWheat/2);
			double suA = seller.findUtility((1+su-bu)*totalCash/2, (1+su-bu)*totalWheat/2);

			if(buA >= buyer.getUtility() && suA >= seller.getUtility()){
				consensus = true;

				buyer.setCash((1+bu-su)*totalCash/2);
				buyer.setWheat((1+bu-su)*totalWheat/2);

				seller.setCash((1+su-bu)*totalCash/2);
				seller.setWheat((1+su-bu)*totalWheat/2);

				amtAgree++;
				rates.println(amtAgree+" "+(originalBC-(1+bu-su)*totalCash/2)/(originalSW-(1+su-bu)*totalWheat/2));
			}
